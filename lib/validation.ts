...
export const consentStepSchema = z.object({
  ...
  dataConsentAccepted: z.literal(true, {
    errorMap: () => ({
      message:
        "You must accept the Data Protection Notice to submit.",
    }),
  }),
});
...
const _surveyBaseSchema = z.object({
  // section 1
...
function applySurveyRefinements<T extends z.infer<typeof _surveyBaseSchema>>(
  data: T,
  ctx: z.RefinementCtx,
) {
  if (data.mainWorkFocus.includes("other") && !data.mainWorkFocusOther) {
...
export const surveySubmissionSchema = _surveyBaseSchema
  .extend({
    submissionSource: z
      .enum(["web", "pwa_offline_sync", "admin_entry"])
      .default("web"),
  })
  .superRefine(applySurveyRefinements);

export const surveyUpdateSchema =
  _surveyBaseSchema.superRefine(applySurveyRefinements);
...
// =============================================================================
// Normalise DB row for form pre-fill (Drizzle numeric returns strings)
// =============================================================================

export function normalizeSurveyForForm(
  data: Record<string, unknown>,
): Partial<SurveySubmission> {
  const copy = { ...data } as Record<string, unknown>;
  if (copy.gpsLatitude != null) copy.gpsLatitude = Number(copy.gpsLatitude);
  if (copy.gpsLongitude != null) copy.gpsLongitude = Number(copy.gpsLongitude);
  if (copy.gpsAccuracyMeters != null)
    copy.gpsAccuracyMeters = Number(copy.gpsAccuracyMeters);
  return copy as Partial<SurveySubmission>;
}
